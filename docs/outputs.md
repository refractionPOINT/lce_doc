...
### Webhook
Output individually each event, detection, audit or deployment through a POST webhook.
...
### SMTP
Output individually each event, detection, audit or deployment through an email.
...
* `type`: this is the stream type you would like to create, one of `event`, `detect`, `audit` or `deployment`.
