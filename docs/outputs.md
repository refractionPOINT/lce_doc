...
* `tailored`: only events specifically flagged for outputs are sent to this stream, see section below.
...
The `tailored` Output stream receives events that are specifically identified through the
D&R `output` [response action](https://doc.limacharlie.io/docs/documentation/ZG9jOjExNjM2MTMz-reference-actions#output).

This allows you to create highly granular Outputs for specific events.

Once your Output is created in the `tailored` stream, you may create a D&R rule that has
an action like:
...
This will cause the single matching event to be sent to the Output in the `name`.